...
export class AssetService {
    ...
    ): Promise<{ asset: Asset; update: AssetUpdate }> {
        try {
            const asset = await this.getAssetById(id);
    ...
    async listAssets(filters: {
        ...
        try {
            ...
            if (filters.producerWalletAddress) {
                queryBuilder.andWhere(
                    "asset.producerWalletAddress = :producerWalletAddress",
                    { producerWalletAddress: filters.producerWalletAddress }
                );
            }

            if (filters.minFundedPercentage) {
                queryBuilder.andWhere(
                    "asset.fundedPercentage >= :minFundedPercentage",
                    {
